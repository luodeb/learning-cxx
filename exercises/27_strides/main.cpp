...
std::vector<udim> strides(std::vector<udim> const &shape) {
    ...
    // Start with stride 1 for the rightmost dimension
    udim current_stride = 1;
    
    // Use reverse iterators to iterate from right to left
    auto strides_it = strides.rbegin();
    auto shape_it = shape.rbegin();
    
    while (strides_it != strides.rend()) {
        *strides_it = current_stride;
        // For next iteration, multiply current stride by the current dimension size
        if (shape_it != shape.rend()) {
